...
`{ "changeName", "changeDir", "schemaName", "contextFiles": { "<artifactId>": ["/abs", ...] }, "progress": {total,complete,remaining}, "tasks": [{id,description,done}], "state": "blocked"|"all_done"|"ready", "missingArtifacts"?, "missingComprehension"?, "comprehension"?: { "required", "passed", "thresholdPercent", "bestScorePercent"?, "questionCount", "questionAllocation": { "plan"?, "specs"?, "design"?, "proposal"?, "tasks"? }, "optionsPerQuestion", "requirementCount", "scenarioCount", "pendingTaskCount", "attempts"? }, "instruction", "references"?, "root" }`.

When comprehension is required and not passed, `state` is `"blocked"` and `missingComprehension` is `true`. The blocked `instruction` references proposal, design, specs, plan, and tasks; agents generate quiz questions using `questionAllocation` (plan receives the majority when present). Each question has `optionsPerQuestion` choices (default 3). Agents MUST present each question to the human developer in chat, wait for answers, and MUST NOT self-answer or call `--record-comprehension-pass` on the user's behalf. Comprehension pass invalidates when delta specs, plan.md, or tasks.md change (fingerprint).
